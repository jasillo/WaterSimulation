...
CFL(void)
{
	float maxv2 = std::max(h*gravity, std::pow(u.infnorm(), 2.0f) + std::pow(v.infnorm(), 2.0f));
...
static inline void solve_distance(float p, float q, float &r)
{
   float d=fmin(p,q)+1;
   if(d>fmax(p,q)) 
      d=(p+q+sqrt(2-std::pow(p-q,2.0)))/2;
   if(d<r) r=d;
}
...
form_preconditioner()
{
   ...
   for(int j=1; j<preconditioner.ny-1; ++j) for(int i=1; i<preconditioner.nx-1; ++i){
      if(marker(i,j)==FLUIDCELL){
		  d = poisson(i, j, 0) - std::pow(poisson(i - 1, j, 1)*preconditioner(i - 1, j), 2.0)
			  - std::pow(poisson(i, j - 1, 2)*preconditioner(i, j - 1), 2.0)
			  - mic_parameter*(poisson(i - 1, j, 1)*poisson(i - 1, j, 2)*std::pow(preconditioner(i - 1, j), 2.0)
				  + poisson(i, j - 1, 2)*poisson(i, j - 1, 1)*std::pow(preconditioner(i, j - 1), 2.0));
